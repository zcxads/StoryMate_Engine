...
# @router.get("/download-ncp/{filename}")
# async def download_ncp_file(filename: str):
#     """NCP URL 정보 반환"""
    
#     try:
#         # 날짜별 폴더를 파일명에서 추출하여 NCP 경로 생성
#         from datetime import datetime
        
#         # 파일명에서 타임스탬프 추출 (예: single_alloy_중성_20241220_143000.mp3)
#         parts = filename.split('_')
#         if len(parts) >= 4:
#             date_part = parts[-2]  # 20241220
#             try:
#                 # 날짜 파리
...
#         else:
#             # 파일명 형식이 예상과 다르면 오늘 날짜 사용
#             date_folder = datetime.now().strftime("%Y%m%d")
        
#         # NCP URL 생성
#         ncp_path = f"{settings.naver_bucket_tts_folder}/{date_folder}/{filename}"
#         ncp_url = f"https://{settings.naver_endpoint_url.replace('https://', '')}/{settings.naver_bucket_name}/{ncp_path}"
...
#             "message": "NCP URL을 사용하여 직접 다운로드하세요."
#         }
        
#     except Exception as e:
#         # NCP 에러 시 로컬 파일로 폴백
#         logger.error(f"⚠️ NCP URL generation failed for {filename}: {str(e)}")
#         file_path = os.path.join(settings.output_dir, filename)
...
#                 "filename": filename,
#                 "ncp_url": None,
#                 "local_download_url": f"/api/v1/tts/download/{filename}",
#                 "download_method": "local_fallback",
#                 "message": "NCP에 엁세스할 수 없어 로컬 다운로드 URL을 제공합니다."
#             }
#         else:
#             raise HTTPException(
#                 status_code=status.HTTP_404_NOT_FOUND,
...
#         if filename.endswith(f".{settings.audio_format}"):
...
# 예시 요청을 위한 샘플 데이터 엔드포인트
# @router.get("/sample")
# async def get_sample_data():
#     """샘플 데이터 조회 (테스트용) - 하트비트 정보 포함"""
...
#         "available_voices": {
#             "gemini": settings.gemini_all_voices,
#             "openai": settings.openai_all_voices,
#         },
...
#         },
#         "realtime_endpoints": {
#             "websocket": "/api/v1/tts/jobs/{job_id}/ws",
#             "sse": "/api/v1/tts/jobs/{job_id}/stream",
#             "polling": "/api/v1/tts/jobs/{job_id}"
#         },
#         "download_endpoints": {
#             # "local_download": "/api/v1/tts/download/{filename}",
#             "ncp_download": "/api/v1/tts/download-ncp/{filename}"
#         },
#         "ncp_features": {
#             "auto_upload": "TTS 파일 생성 시 자동으로 NCP에 업로드",
#             "ncp_url_included": "API 응답에 NCP URL 포함",
#             "fallback_support": "NCP 오류 시 로컬 파일로 폴백"
#         },
#         "heartbeat_features": {
#             "websocket": {
